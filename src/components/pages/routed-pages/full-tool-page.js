...
function ToolPage() {
  ...
  useEffect(() => {
    async function fetchContent() {
      const response = await fetch(process.env.REACT_APP_BACKEND_URL + `/tools/id/${tid}`);
    ...
    <section className="page-section full-tool-page">
      ...
      {content && (
        <div className="full-tool-content">
          ...
          <img
            src={content.toolImage ? content.toolImage : "https://images.unsplash.com/photo-1460925895917-afdab827c52f?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1115&q=80"}
            alt="tool"
            ...
            {content.scienceURL.map((item) => {
              return (
                <li key={content.scienceURL.indexOf(item)}>
                  <a className="source-link-full" href={item.url} target={"_blank"} rel="noreferrer" >{item.title}</a>
                </li>
              );
            })}
          </ul>
